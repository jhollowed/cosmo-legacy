...
def group_halos(catalog = 0, clusterMass = 1e14, disruption = 0.05, massThresh = 10**11.27, 
                process = 0):
	...
	outputFile = h5py.File('{}/haloCores_{}processed2.hdf5'.format(saveDest, p_prefix), 'w')
	...
	for j in range(len(allCores)):
		...
		for k in range(len(haloTags)):
			...
			if(process):
				...
				adMask = core_radii < 	disruption
				massMask = core_infallMass > massThresh
				coreMask = np.logical_and(radMask, massMask)
                                #unwrap positions
                                core_pos = np.array([gio.gio_read(cores, p) for p in ['x','y','z']])
