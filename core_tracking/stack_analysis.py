...
def stack_segregation(n_draws=21, min_r = 0.1, max_r=3, max_z=1.5, catalog=0, velComp =2, norm=0, 
                      processed = True):
    '''
    ...
    :param min_r: minimum normalized radial distance of which to draw cores from (default=1)
    ...
    if(velComp == ''): suff = 'z{}r{}-{}n{}'.format(max_z, min_r, max_r, norm[1])
    else: suff = 'z{}r{}-{}n{}v{}'.format(max_z, min_r, max_r, norm[1], velComp[1])
    ...
    segrStackFile.attrs.create('min_(r/r200)', min_r)
    ...
    for n  in range(len(zStacks)):
        ...
        step = stepNums[n]
        stack = zStacks[n]
        rMask = np.logical_and(stack['r_norm'][:] <= max_r, stack['r_norm'][:] >= min_r)
        z = zs[n]
        print('\n\n----- Working on stack at z = {} ({}/{})-----'.format(z, n+1, len(zStacks))) 
        ...
        for prop in segrProp:
            ...
            for j in range(n_draws):
                ...
                lower_pop = int(math.ceil(N_tot*(1-fracs[j])))
                upper_pop = int(math.floor(N_tot*fracs[j]))
                sample = np.concatenate([lower_sample[0:lower_pop], upper_sample[0:upper_pop]])
                v_sample = stack['v{}{}'.format(velComp, norm)][:][sample]

                # find the dispersion of the cores in this bin, and associated errors, via 
                # bootstrap resampling 
                [vDisp_avg, vDisp_err] = stat.bootstrap_bDispersion(v_sample, 
                                                                    size=len(v_sample)/2, repl = False)
...
def double_segregation(n_draws = 15, min_r = 0, max_r = 3, max_z = 0.5, catalog=0, velComp=2, 
                       norm = 0, processed = True):

    '''
    The exact same function as stacked_segregation, except it produces two segregation datasets
    for a particular segregation property, by preforming the analysis on two sides of the median
    ...
    # create new hdf5 file to hold segregation data
    segrStackFile = h5py.File('{}/segregatedCores_double2_{}processed_{}.hdf5'
                              .format(corePath, processPrefix, suff), 'w')
    print('\nRead data from core catalog and created file for segregation')
    print('Using {} velocity with {} normalization'.format(catalogName, norm))
    ...
    # list each core property to be segregated
    segrProp = ['radius', 'time_since_infall']
    varyProp = 'infall_mass'
    ...
    for n  in range(len(zStacks)):
        ...
        for prop in segrProp:
            ...
            for k in range(2):
                print('----------starting variable split {}----------'.format(k+1))

                # the number of cores used in the segregation analysis, 
                # N_tot should not exceed the minimum of these two populations
                vary_sample = np.where(stack['{}_segr'.format(varyProp)][:] == k)[0]
                prop_sample = stack['{}_segr'.format(prop)][:][vary_sample]
                allv_sample = stack['v{}{}'.format(velComp, norm)][:][vary_sample]
                lower_sample = np.where(prop_sample == 0)[0]
                upper_sample = np.where(prop_sample == 1)[0]
                N_tot = min(len(lower_sample), len(upper_sample))
                pdb.set_trace()
                continue
                ...
                print('saving segregation data') 
                dataFile = segrStackFile.create_group('step_{}/{}_segr/vary_{}_{}'.format(step, 
                                                                            prop, varyProp, k+1))
                dataFile.create_dataset('pcen_lowerPop', data = fracs)
                dataFile.create_dataset('bin_vDisp', data = bin_dispersions)
                dataFile.create_dataset('bin_vDispErr', data = bin_disp_errors)
...
def segregation_binning(n_bins=15, max_r=3, min_r = 0.1, max_z=0.5, catalog=0, velComp =2, norm=0, 
                        processed = True):
    '''
    ...
    :param max_r: maximum normalized radial distance of which to draw cores from (default=3)
    :param min_r: minimum normalized radial distance of which to draw cores from (default=0.1)
    :param max_z: the maximum redshift from which to draw cores
    :param catalog: version of core velocities to use (0=biweight, 1=median, 2=central particle)
    :param velComp: which velocity component to use (0=radial, 1=tangent, or 2=total velocity vector)
    :param norm: which velocity normalization to use (0=coreNorm, 1=haloNorm)
    ...
    for n  in range(len(zStacks)):
        ...
        for prop in segrProp:
            ...
            # note - the bin edges are defined by SORTED INDEX, not by VALUE of the poperty data. 
            # This is to avoid a crash in the event that you have, say, property values that are
            # all equal for more than the span of a bin
            sortMask = np.argsort(stack[prop][:])
            propData = stack[prop][:][sortMask]
            propIdx = np.linspace(0, len(propData)-1, len(propData)).astype(int)
            bin_edges = np.percentile(propIdx, np.linspace(0, 100, n_bins+1))
            bin_centers = bin_edges[:-1] + np.diff(bin_edges)/2

            bin_edges_vals = propData[np.round(bin_edges).astype(int)]
            bin_center_vals = propData[np.round(bin_centers).astype(int)]
            bin_widths = np.diff(bin_edges_vals)
            
            bin_dispersions = np.zeros(n_bins)
            bin_disp_errors = np.zeros(n_bins)

            for j in range(n_bins):
                print('Working on bin {} ({:.2f} > {} > {:.2f})'.format(j, bin_edges_vals[j], prop, 
                                                                        bin_edges_vals[j+1]))
                if(j == 0):
                    binMask = (propIdx >= bin_edges[j]) & (propIdx <= bin_edges[j+1])
                else:
                    binMask = (propIdx > bin_edges[j]) & (propIdx <= bin_edges[j+1])

                bin_vs = stack['v{}{}'.format(velComp, norm)][:][sortMask][binMask]
            ...
            print('saving segregation data')    
            dataFile = segrStackFile['step_{}'.format(step)]['{}_segr'.format(prop)]
            dataFile.create_dataset('bin_center', data = bin_center_vals)
