...
def allTests(lcDir_interp, lcDir_extrap, snapshotDir, histFigDir, trajectoryDir, trajectoryPlotDir, 
    ...
    if(pmode == 'halos'):
        ...
        vt.findDuplicates(lcDir_interp, steps=[453, 464], lcSuffix='interp', outDir=duplicateDir,
                          mode=pmode)
        vt.findDuplicates(lcDir_extrap, steps=[453, 464], lcSuffix='extrap', outDir=duplicateDir,
                          mode=pmode)

    print('\n\n ==== Plotting duplicates validation test =====')
    vt.compareDuplicates(duplicateDir, steps=[453, 464], lcSuffix=['interp', 'extrap'],
